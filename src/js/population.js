...
class Population{
	constructor(size){
		...
		for(let i = 0; i < size; i++){
			this.population.push(new FlappyBird(i));
	...
	updateAlive(){
		for(let i = 0; i < this.population.length; i++){
			...
			if(!this.population[i].dead){
				if(this.population[i].direction){
					//wait until it goes down again
				} else {
					this.population[i].down();
				}
			}
			//draw
			this.population[i].draw('yellow');
			//die
			this.population[i].checkdie();
		}
	}
